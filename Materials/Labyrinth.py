...
class Labyrinth:
    ...
    def case_number_arround(self,x:int,y:int)->list:
        case_arround = []
        try:
            if self.board[x-1][y].k_number != self.board[x][y].k_number and x-1 >=0:
                case_arround.append([x-1,y])
        except:
            pass
        try:
            if self.board[x+1][y].k_number != self.board[x][y].k_number and x+1 <self.length:
                case_arround.append([x+1,y])
        except:
            pass
        try:
            if self.board[x][y-1].k_number != self.board[x][y].k_number and y-1 >=0:
                case_arround.append([x,y-1])
        except:
            pass
        try:
            if self.board[x][y+1].k_number != self.board[x][y].k_number and y+1 <self.length:
                case_arround.append([x,y+1])
        except:
            pass
        return case_arround
